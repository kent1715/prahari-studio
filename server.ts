...
app.post("/api/generate-ideas", async (req, res) => {
  const { niche, language, style, audience } = req.body;
  ...
  if (!client) {
    ...
    return res.json({
      ...
      message: "Menggunakan mode simulasi (Kunci API Gemini tidak dikonfigurasi)."
    });
  }
...
app.post("/api/generate-script", async (req, res) => {
  const { title, niche, style, language, voiceEmotion } = req.body;
  const client = getGeminiClient();
  ...
  if (!client) {
    return res.json({
      ...
      message: "Menggunakan naskah simulasi berkualitas tinggi karena Kunci API Gemini tidak dikonfigurasi."
    });
  }
...
app.post("/api/generate-storyboard", async (req, res) => {
  const { fullText, style } = req.body;
  const client = getGeminiClient();

  if (!client) {
    // High quality mock storyboard scenes
    return res.json({
      success: true,
      mode: "simulation",
      scenes: [
        {
          ...
          prompt: "gurun pasir kuno yang diterangi cahaya bulan redup, reruntuhan kuil batu terbengkalai, badai debu tipis, gaya dokumenter sejarah",
